...
let overview = (pokemon) => {
  ...
  document.getElementById("cp").innerHTML += stat["max-cp"];
  document.getElementById("candy").innerHTML += pokemon["buddy-distance-km"];
};
...
let getPokemon = (pokemon) => {
  ...
  function drawBasic() {
    ...
    var options = {
      title: "BASE STATS OF POKEMONS",
      chartArea: { width: "60%", height: "50%" },
      hAxis: {
        title: "HP Healt Points,    ATK  Attack,   DEF Defense,   STAM stamina",
        minValue: 0,
      },
      vAxis: {
        title: "Stats",
